  ...
  try {
    if (isTeacher && Array.isArray(teacherIDs) && teacherIDs.length > 0) {
      ...
      if (validForms.length > 0) {
        return res.status(201).json({ message: 'Forms created successfully!', forms: validForms });
      }
    }

    return res.status(400).json({ message: 'Invalid data. Teacher IDs are required.' });
  } catch (error) {
...
exports.getObserverDashboard = async (req, res) => {
  ...
  try {
    ...
    if (forms.length === 0) {
      return res.status(404).json({ message: 'No forms available for filling.' });
    }
...
exports.EditUpdate = async (req, res) => {
    ...
    try {
        ...
        const updateData = { ...(observerForm && { observerForm }), ...(teacherForm && { teacherForm }) };
